...
SET @replicationdb = N'SP.Provider.DB';
SET @publisherlogin = N'sa';
SET @publisherpassword = STRING_ESCAPE(N'S3cur3P@ssW0rd!', 'json');
SET @publicationName = N'ProviderTransactionalRepl'
...
EXEC sp_addpublication @publication = @publicationName
    ,@description = N'Transactional publication of database ''SP.Provider.DB'' from Publisher ''''.'
    ,@retention = 0
    ,@allow_push = N'true'
